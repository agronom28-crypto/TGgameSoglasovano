...
function updateDust(dt) {
  dustParticles.forEach(p => {
    p.x += p.vx; p.y += p.vy;
    p.vy += 0.08; p.r *= 0.97; p.life -= p.decay;
  });
...
function triggerBoostFlash(px, py) {
  ...
  // 2. взрыв частиц — звёздочки вокруг персонажа
  for (let i = 0; i < 24; i++) {
    ...
    burstParticles.push({
      ...
      hue: 40 + Math.random() * 30,   // золотистый / оранжевый
    });
  }
...
function updateFlash(dt) {
  if (flash.active) {
    flash.life -= dt * 6;   // быстро гаснет за ~0.17с
    if (flash.life <= 0) { flash.active = false; flash.life = 0; }
  }
...
function drawFlash() {
  ...
  burstParticles.forEach(p => {
    ...
    ctx.fill();
    // звёздочка-лучик: линия от центра
...
function drawCharacter(x, ground, phase, isDucking, boost) {
  ...
  if (isDucking) {
    ...
    ctx.beginPath(); ctx.arc(bx, ground - 44, 14, 0, Math.PI * 2);
  ...
  const bx = x + 22, baseY = ground - bodyBob;
  ...
  ctx.beginPath(); ctx.moveTo(-5, 22); ctx.lineTo(5, 22); ctx.lineTo(0, 30);
  ctx.closePath(); ctx.fill();
  ctx.restore();

  ctx.save(); ctx.translate(bx - 6, baseY - 24); ctx.rotate((legSwing * Math.PI) / 180);
  ctx.fillStyle = '#1a1a2e'; ctx.fillRect(-5, 0, 10, 24);
  ctx.translate(0, 24); ctx.rotate((-legSwing * 0.6 * Math.PI) / 180);
  ctx.fillRect(-4, 0, 9, 20); ctx.fillStyle = '#333'; ctx.fillRect(-6, 18, 16, 7);
  ctx.restore();
  ...
  ctx.fillStyle = suit;
  ctx.beginPath(); ctx.roundRect(bx - 16, baseY - 62, 32, 38, 6); ctx.fill();
  ...
  ctx.fillStyle = skin; ctx.fillRect(-4, 18, 8, 10); ctx.restore();

  ctx.save(); ctx.translate(bx + 16, baseY - 56); ctx.rotate((-armSwing * Math.PI) / 180);
  ctx.fillStyle = suit; ctx.fillRect(-4, 0, 9, 20);
  ctx.fillStyle = skin; ctx.fillRect(-3, 18, 8, 10); ctx.restore();

  ctx.beginPath(); ctx.arc(bx, baseY - 72, 14, 0, Math.PI * 2);
  ctx.fillStyle = skin; ctx.fill();
  ctx.beginPath(); ctx.arc(bx, baseY - 80, 14, Math.PI, 0);
  ctx.fillStyle = '#4E342E'; ctx.fill();
  ctx.beginPath(); ctx.arc(bx + 5, baseY - 73, 3, 0, Math.PI * 2);
  ...
  ctx.beginPath(); ctx.arc(9, -1, 5, Math.PI, 0); ctx.stroke();
  ctx.restore();
...
function update(dt) {
  ...
  if (!state.isJumping && !state.isDucking) {
    const crossed = (ph, prev, cur) =>
      (prev < ph && cur >= ph) || (prev > cur && (cur >= ph || prev < ph));
...
function checkCollisions() {
  ...
  state.boosts.forEach((b, i) => {
    if (b.lane === state.lane && b.x < px + PLAYER_W && b.x + 60 > px) {
      ...
      triggerBoostFlash(state.x, state.y);   // ← здесь!
    }
  });
}
...
function draw() {
  ...
  // Тряска камеры — сдвигаем весь canvas context
  ctx.save();
  ...
  state.boosts.forEach(b => {
    ...
    ctx.fillStyle = '#FF6B00'; ctx.font = '20px sans-serif';
    ctx.fillText('⚡', b.x + 15, GROUND_Y + 50);
  });

  state.obstacles.forEach(o => {
    if (o.type === 'dog') o.lanes.forEach(l => {
  ...
  // Вспышка и частицы — поверх всего
  drawFlash();
