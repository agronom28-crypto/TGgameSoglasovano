...
function spawnObstacle() {
  ...
  state.obstacles.push({
    ...
    lanes,
    x: canvas.width + 60,
    y: type === 'ball' ? GROUND_Y - 120 : GROUND_Y,
  });
}
...
function update(dt) {
  ...
  if (state.boostTimer > 0) {
    state.boostTimer -= dt;
    if (state.boostTimer <= 0) state.boost = false;
  }
  ...
  if (Date.now() - state.lastObstacleTime > 1800) {
    spawnObstacle();
    state.lastObstacleTime = Date.now();
  }
  if (Date.now() - state.lastBoostTime > 4000) {
    spawnBoostStrip();
...
function checkCollisions() {
  ...
  state.obstacles.forEach(o => {
    ...
    if (o.type === 'dog') {
      if (o.lanes.includes(state.lane)) { endGame('hit'); }
    }
    if (o.type === 'pit') {
      if (o.lanes.includes(state.lane) && !state.isJumping) { endGame('hit'); }
    }
    if (o.type === 'ball') {
      if (o.lanes.includes(state.lane) && !state.isDucking && pTop < o.y) {
        endGame('hit');
      }
    }
  });
...
function draw() {
  ...
  state.obstacles.forEach(o => {
    if (o.type === 'dog') {
      o.lanes.forEach(l => {
        ...
        ctx.fillText('🐕', LANE_WIDTH * l + LANE_WIDTH/2 - 20, GROUND_Y - 5);
      });
    }
    if (o.type === 'pit') {
      o.lanes.forEach(l => {
        ctx.fillStyle = '#1a0000';
        ctx.fillRect(LANE_WIDTH * l + 5, GROUND_Y - 5, LANE_WIDTH - 10, 40);
        ctx.font = '24px sans-serif';
        ctx.fillText('🕳️', LANE_WIDTH * l + LANE_WIDTH/2 - 12, GROUND_Y + 30);
      });
    }
    if (o.type === 'ball') {
  ...
  const ph = state.isDucking ? PLAYER_H * 0.5 : PLAYER_H;
  ctx.font = '40px sans-serif';
  ctx.fillText('🏃', state.x - 5, state.y - (state.isDucking ? 10 : 5));
...
function endGame(reason) {
  ...
  if (reason === 'finish') {
    el.innerHTML = `🏁 Финиш!<br>Время: ${elapsed.toFixed(1)}с`;
    if (window.Telegram?.WebApp) {
      Telegram.WebApp.sendData(JSON.stringify({ score, time: elapsed }));
    }
